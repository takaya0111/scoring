...
max_score_2013=["10","25","9","18"]
for i,j in enumerate(max_score_2013):
    max_score_2013[i]=j.translate(str.maketrans({chr(0x0021 + i): chr(0xFF01 + i) for i in range(94)}))
...
for a in os.listdir(before_file_path):
    if ".pdf" in a:
        ...
        for i,j in enumerate(scores):
            if i>0:
                draw.text((1750, 600+50*(i)), "大問 "+str(i+2)+' : '+j+"／"+max_score_2013[i], font = font, fill = (0,0,255))
            else:
                draw.text((1750, 600+50*(i)), "大問 "+str(i+1)+' : '+j+"／"+max_score_2013[i], font = font, fill = (0,0,255))
        ...
        for j,i in enumerate(before):
            before[j]=before[j].resize((841,595),Image.LANCZOS)
#            before[j]=before[j].filter(ImageFilter.UnsharpMask(radius=0.2))


            #pdfファイルの保存名を指定
        pdf_name = after_path + "/" +  "PDF_" + "0" + ".pdf"
        before[0].save(pdf_name,quality=95)
        merge = PyPDF2.PdfFileMerger()

        merge.append(pdf_name)
#        os.remove(pdf_name)
#        for i in range(2,7):

        merge.append(a_path,pages=PyPDF2.pagerange.PageRange('1:'))
