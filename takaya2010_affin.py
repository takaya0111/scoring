...
def check_support(im,x,y,ox,oy,point=0):
    ...
    if img_1.sum()/((img_1.shape)[0]*(img_1.shape[1]*3)) <200:
        red_circle(im,oy,ox)    
        score += point
    else:
        batu(im,oy,ox)
...
#     scores=[]
    ...
    check_support(img,685,380,685,210)
    check_support(img,740,268,745,210)
    check_support(img,800,295,800,210)
    check_support(img,855,323,855,210)
    check_support(img,915,295,915,210)
    check_support(img,970,323,970,210)
    check_support(img,1030,351,1030,210)
    check_support(img,1085,268,1085,210)
    check_support(img,1140,290,1140,210)
    check_support(img,1195,263,1195,210)
    check_support(img,1250,345,1250,210)
    check_support(img,1305,290,1305,210)
    check_support(img,1365,263,1365,210)
    check_support(img,1420,345,1420,210)
    
    check_support(img,630,765,630,715)
    check_support(img,685,768,685,715)
    check_support(img,740,825,740,715)
    check_support(img,798,822,798,714)
    check_support(img,855,768,855,714)
    check_support(img,910,768,910,714)
    check_support(img,970,768,970,713)
    check_support(img,1030,793,1030,713)
    check_support(img,1085,793,1085,713)
    check_support(img,1140,820,1140,712)
    check_support(img,1200,850,1200,712)
    check_support(img,1255,765,1255,712)
    img[:30,:]=255
    img[1600:,:]=255
    img[:,:30]=255
    img[:,2300:]=255
...
for a in os.listdir(before_file_path):
    if ".pdf" in a:
        ...
        ax1,ax2=int(1323-15),int(1323+15)
        ...
        dx1,dx2=int(1705-30),int(1705+30)
        dy1,dy2=int(460-30),int(460+30)
        d=140
        feature = []
        feature_stock=[]
